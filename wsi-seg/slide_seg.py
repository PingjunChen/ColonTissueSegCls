...
def test_slide_seg(args):
    ...
    for num, cur_slide in enumerate(slide_names):
        ...
        mask_img = io.imread(mask_path) / 255
        # split and predict
        coors_arr = wsi_stride_splitting(slide_img.shape[0], slide_img.shape[1], patch_len=args.patch_len, stride_len=args.stride_len)
        patch_arr, mask_arr, wmap = gen_patch_mask_wmap(slide_img, mask_img, coors_arr, plen=args.patch_len)
        patch_dset = PatchDataset(patch_arr, mask_arr)
        ...
        for ind, (patches, masks) in enumerate(patch_loader):
        ...
        pred_dice = np.sum(intersection) / (np.sum(mask_img)+np.sum(slide_pred)-np.sum(intersection))
